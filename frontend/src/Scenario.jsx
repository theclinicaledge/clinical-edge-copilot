...
// ─── Copilot block — one accent (teal), per design-system.md §6.2/§6.8:
// the marketing preview mirrors the real product's actual section styling. ──
function CopilotBlock({ label, accent = "var(--ce-teal-deep)", bg = "rgba(10,191,188,0.06)", border = "rgba(10,191,188,0.22)", children }) {
  return (
    <div style={{
      background: bg,
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--ce-navy-900)", fontFamily: "'Inter', sans-serif", color: "var(--ce-text-light)" }}>
      <style>{`
        ...
        .sc-nav-link:hover { color: var(--ce-teal); }
        .sc-nav-link:focus-visible { outline: 2px solid var(--ce-teal); outline-offset: 2px; }
        .sc-nav-cta {
        ...
        .sc-nav-cta:hover { border-color: rgba(10,191,188,0.30); background: rgba(10,191,188,0.06); }
        .sc-nav-cta:active { transform: scale(0.98); transition-duration: 60ms; }
        .sc-nav-cta:focus-visible { outline: 2px solid var(--ce-teal); outline-offset: 2px; }
        .sc-scenario-chip {
          transition: border-color var(--ce-dur-fast) var(--ce-ease-out), background var(--ce-dur-fast) var(--ce-ease-out), color var(--ce-dur-fast) var(--ce-ease-out), transform var(--ce-dur-fast) var(--ce-ease-out), opacity var(--ce-dur-fast) var(--ce-ease-out);
        }
        .sc-scenario-chip:hover { border-color: var(--ce-teal); background: rgba(10,191,188,0.06); color: var(--ce-text-light); }
        .sc-scenario-chip:active { transform: scale(0.97); opacity: 0.85; transition-duration: 60ms; }
        .sc-scenario-chip:focus-visible { outline: 2px solid var(--ce-teal); outline-offset: 2px; }
        .sc-btn-primary {
          transition: background var(--ce-dur-fast) var(--ce-ease-out), transform var(--ce-dur-fast) var(--ce-ease-out);
        }
        .sc-btn-primary:hover { background: var(--ce-teal-deep); }
        .sc-btn-primary:active { transform: scale(0.98); transition-duration: 60ms; }
        .sc-btn-primary:focus-visible { outline: 2px solid var(--ce-teal); outline-offset: 2px; }
        .sc-btn-ghost {
          transition: border-color var(--ce-dur-fast) var(--ce-ease-out), color var(--ce-dur-fast) var(--ce-ease-out), transform var(--ce-dur-fast) var(--ce-ease-out);
        }
        .sc-btn-ghost:hover { border-color: rgba(10,191,188,0.30); color: var(--ce-text-light); }
        .sc-btn-ghost:active { transform: scale(0.98); transition-duration: 60ms; }
        .sc-btn-ghost:focus-visible { outline: 2px solid var(--ce-teal); outline-offset: 2px; }
        .sc-step-dot { transition: background var(--ce-dur-base) var(--ce-ease-out), border-color var(--ce-dur-base) var(--ce-ease-out), color var(--ce-dur-base) var(--ce-ease-out); }
        .sc-step-connector { transition: background var(--ce-dur-base) var(--ce-ease-out), opacity var(--ce-dur-base) var(--ce-ease-out); }
      `}</style>
      ...
      <nav style={{
        ...
        <button onClick={onBack} className="sc-nav-link" style={{
        ...
        <button onClick={onEnterApp} className="sc-nav-cta" style={{
      ...
      <main style={{ maxWidth: 720, margin: "0 auto", padding: "60px clamp(20px, 5vw, 40px) 100px" }}>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  padding: "8px 16px",
                  borderRadius: "var(--ce-r-md)",
                  border: `1px solid ${active ? "var(--ce-teal)" : "var(--ce-line-navy)"}`,
                  background: active ? "rgba(10,191,188,0.10)" : "transparent",
                  color: active ? "var(--ce-teal)" : "var(--ce-text-dim)",
                  cursor: "pointer",
                }}
              >
                Scenario {s.id}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 40 }}>
          {[1, 2, 3].map(n => (
            <div key={n} style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div className="sc-step-dot" style={{
                width: 28, height: 28, borderRadius: "50%",
                background: step >= n ? "var(--ce-teal)" : "transparent",
                border: `1.5px solid ${step >= n ? "var(--ce-teal)" : "var(--ce-line-navy)"}`,
                display: "flex", alignItems: "center", justifyContent: "center",
                fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, fontWeight: 700,
                color: step >= n ? "var(--ce-text-dark)" : "var(--ce-text-dim)",
                flexShrink: 0,
              }}>
                {n}
              </div>
              {n < 3 && (
                <div className="sc-step-connector" style={{
                  width: 32, height: 1.5,
                  background: step > n ? "var(--ce-teal)" : "var(--ce-line-navy)",
          ...
          <span style={{
            fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, color: "var(--ce-text-dim)",
        ...
        {step === 1 && (
          <div>
            ...
            <h1 style={{
              ...
              color: "var(--ce-text-light)", letterSpacing: "-1.2px", lineHeight: 1.1,
              margin: "0 0 32px",
            }}>
              {sc.stepOneTitle}
            </h1>
            ...
            <div style={{
              background: "var(--ce-navy-700)", border: "1px solid var(--ce-line-dark)",
              borderRadius: "var(--ce-r-lg)", padding: "32px 30px", marginBottom: 32,
            }}>
              {/* Unit badge */}
              <div style={{
                display: "inline-flex", alignItems: "center", gap: 8,
                background: "rgba(10,191,188,0.10)", border: "1px solid rgba(10,191,188,0.22)",
                borderRadius: "var(--ce-r-sm)", padding: "6px 12px", marginBottom: 24,
              }}>
                <div style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--ce-teal)" }} />
                <span style={{
                  fontFamily: "'IBM Plex Mono', monospace", fontSize: 10,
                  color: "var(--ce-teal)", letterSpacing: "var(--ce-track-eyebrow)", textTransform: "uppercase",
                }}>
                  {sc.unit}
                </span>
              </div>
              ...
              {scenarioIndex === 0 && (
                <>
                  <p style={{ fontSize: 15, color: "var(--ce-text-light-body)", lineHeight: 1.82, margin: "0 0 18px" }}>
                    You're doing routine rounding and walk into Room 14. <strong style={{ color: "var(--ce-text-light)" }}>Mr. Okafor, 67M, POD #2 from elective right hemi-colectomy.</strong> Uncomplicated surgery, no real history except mild hypertension — controlled on one med. He was fine at midnight.
                  </p>
                  <p style={{ fontSize: 15, color: "var(--ce-text-light-body)", lineHeight: 1.82, margin: "0 0 18px" }}>
                    He's awake. Sitting up. Looks at you when you walk in.
                  </p>
                  <p style={{ fontSize: 15, color: "var(--ce-text-light)", fontWeight: 600, lineHeight: 1.82, margin: "0 0 24px" }}>
                    But something's off.
                  </p>
                  <p style={{ fontSize: 15, color: "var(--ce-text-light-body)", lineHeight: 1.82, margin: "0 0 28px" }}>
                    He's not distressed. He's not clutching his chest. He's not telling you anything is wrong. He just looks... pale. Quieter than earlier. When you ask how he's feeling he says <em style={{ color: "var(--ce-text-light)" }}>"a little tired, probably just the surgery."</em>
                  </p>
                </>
              )}
              ...
              {scenarioIndex === 1 && (
                <>
                  <p style={{ fontSize: 15, color: "var(--ce-text-light-body)", lineHeight: 1.82, margin: "0 0 18px" }}>
                    Your CNA pulls you aside during afternoon rounds. <strong style={{ color: "var(--ce-text-light)" }}>Mr. Torres, 58M, admitted 3 days ago for community-acquired pneumonia.</strong> On IV ceftriaxone and azithromycin. Yesterday's WBC was 9.8 — down from 16.2 on admission. The team was talking about stepping him to oral antibiotics, maybe discharge tomorrow.
                  </p>
                  <p style={{ fontSize: 15, color: "var(--ce-text-light-body)", lineHeight: 1.82, margin: "0 0 18px" }}>
                  ...
                  <p style={{ fontSize: 15, color: "var(--ce-text-light-body)", lineHeight: 1.82, margin: "0 0 28px" }}>
                    His answers come slowly. He says he feels <em style={{ color: "var(--ce-text-light)" }}>"just tired."</em> He ate breakfast but refused lunch — says his stomach feels off. New crackles on the right, worse than yesterday. No chest pain. No obvious source change.
                    <br /><br />
              ...
              {scenarioIndex === 2 && (
                <>
                  <p style={{ fontSize: 15, color: "var(--ce-text-light-body)", lineHeight: 1.82, margin: "0 0 18px" }}>
                    You walk into Room 8 after seeing a tech note from 20 minutes ago: <em style={{ color: "var(--ce-text-light)" }}>"Patient c/o chest discomfort, resolved."</em> <strong style={{ color: "var(--ce-text-light)" }}>Mrs. Patel, 64F, admitted yesterday for hypertensive urgency.</strong> HTN, DM2, former smoker — quit 10 years ago. No known cardiac history. She was being transitioned from IV nicardipine to oral meds and doing well.
                  </p>
                  <p style={{ fontSize: 15, color: "var(--ce-text-light-body)", lineHeight: 1.82, margin: "0 0 18px" }}>
              ...
              <div style={{
                background: "rgba(255,255,255,0.04)", border: "1px solid var(--ce-line-dark)",
                borderRadius: "var(--ce-r-md)", padding: "20px 22px", marginBottom: 20,
              }}>
                <div style={{
                  fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, color: "var(--ce-text-dim)",
                  textTransform: "uppercase", letterSpacing: "var(--ce-track-eyebrow)", marginBottom: 14,
                }}>
                  Current Vitals
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: "10px 24px" }}>
                  {sc.vitals.map(v => (
                    <div key={v.label}>
                      <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, color: "var(--ce-text-dim)", letterSpacing: "0.5px", marginBottom: 3 }}>
                        {v.label}
                      </div>
                      <div style={{ fontSize: 16, fontWeight: 700, color: v.was ? "var(--ce-urgency-high-dark)" : "var(--ce-text-light)", letterSpacing: "-0.3px" }}>
                        {v.value}
                      </div>
                      {v.was && (
                        <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, color: "var(--ce-text-dim)", marginTop: 2 }}>
                          {v.was}
                        </div>
                      )}
                    </div>
              ...
              {scenarioIndex === 0 && (
                <p style={{ fontSize: 14, color: "var(--ce-text-light-body)", lineHeight: 1.72, margin: 0 }}>
                  Abdomen soft. Wound dressing clean and dry. Last Hgb <strong style={{ color: "var(--ce-text-light)" }}>11.2 on POD #1.</strong>
                </p>
              )}
              {scenarioIndex === 1 && (
                <p style={{ fontSize: 14, color: "var(--ce-text-light-body)", lineHeight: 1.72, margin: 0 }}>
                  Yesterday's WBC: <strong style={{ color: "var(--ce-text-light)" }}>9.8</strong> (improving). This morning's labs not yet resulted. Skin warm, no rash. Cap refill 3 seconds.
                </p>
              )}
              {scenarioIndex === 2 && (
                <p style={{ fontSize: 14, color: "var(--ce-text-light-body)", lineHeight: 1.72, margin: 0 }}>
                  Tech-performed 12-lead on file — <strong style={{ color: "var(--ce-text-light)" }}>"no obvious ST changes"</strong> per tech read. You haven't reviewed it yet. No current orders for troponin repeat. Skin slightly pale, mild diaphoresis on exam.
                </p>
              )}
            </div>

            <div style={{ display: "flex", justifyContent: "flex-end" }}>
              <button className="sc-btn-primary" onClick={() => setStep(2)} style={btnPrimary}>
                Think it through →
              </button>
            </div>
          </div>
        ...
        {step === 2 && (
          <div>
            ...
            <p style={{ fontSize: 15, color: "var(--ce-text-light-body)", lineHeight: 1.72, margin: "0 0 36px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 40 }}>
              {sc.questions.map((q, i) => (
                <div key={i} style={{
                  background: "var(--ce-navy-700)", border: "1px solid var(--ce-line-dark)",
                  borderLeft: "3px solid var(--ce-teal)", borderRadius: "var(--ce-r-lg)",
                  padding: "22px 24px", display: "flex", gap: 18, alignItems: "flex-start",
                }}>
                  <div style={{
                    fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, fontWeight: 600,
                    color: "var(--ce-teal)", opacity: 0.6, flexShrink: 0, paddingTop: 2,
                  }}>
                    {String(i + 1).padStart(2, "0")}
                  </div>
                  <p style={{ fontSize: 15, color: "var(--ce-text-light)", lineHeight: 1.65, margin: 0, fontWeight: 500 }}>
                    {q}
                  </p>
                </div>
              ))}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
              <button className="sc-btn-ghost" onClick={() => setStep(1)} style={btnGhost}>
                ← Back
              </button>
              <button className="sc-btn-primary" onClick={() => setStep(3)} style={btnPrimary}>
        ...
        {step === 3 && (
          <div>
            ...
            <p style={{ fontSize: 14, color: "var(--ce-text-dim)", lineHeight: 1.65, margin: "0 0 32px" }}>
            ...
            <div style={{ marginBottom: 28 }}>
              ...
              {scenarioIndex === 0 && (
                <>
                  ...
                  <CopilotBlock label="Early Action Priorities">
                    <ul style={{ margin: 0, paddingLeft: 18, display: "flex", flexDirection: "column", gap: 8 }}>
                      <li>IV access — confirm patent, get a second large-bore if only one in</li>
                      <li><strong style={{ color: "var(--ce-text-light)" }}>Notify the surgical resident now,</strong> not when it's worse — lead with the trend: <em>"He was 128/78 at midnight, now 102/64, HR up to 98, UO dropping — I'm concerned."</em></li>
              ...
              {scenarioIndex === 1 && (
                <>
                  <CopilotBlock label="Pattern Recognition">
                    New fever, tachycardia, hypotension, worsening SpO₂, and a subtle mental status change — in a patient who was <strong style={{ color: "var(--ce-text-light)" }}>improving.</strong> That pattern is sepsis physiology until proven otherwise. The pneumonia may be progressing, a new source may have developed, or this is treatment failure. The improving WBC from yesterday is now working against you — it'll create cognitive bias in anyone you call.
                  </CopilotBlock>
                  <CopilotBlock label="What Concerns Me Most">
                    The mental status change. <em>Subtle is dangerous.</em> A patient who "seems slow" or "just tired" in the context of fever, tachycardia, and hypotension is showing early signs of septic encephalopathy — that's organ dysfunction. That's not just infection. <strong style={{ color: "var(--ce-text-light)" }}>That's sepsis.</strong> And the fact that yesterday's labs were reassuring makes this moment easier to miss and harder to escalate.
                  </CopilotBlock>
                  <CopilotBlock label="Assess Next — In This Order">
                    <ol style={{ margin: 0, paddingLeft: 18, display: "flex", flexDirection: "column", gap: 8 }}>
                  ...
                  <CopilotBlock label="Early Action Priorities">
                    <ul style={{ margin: 0, paddingLeft: 18, display: "flex", flexDirection: "column", gap: 8 }}>
                      <li><strong style={{ color: "var(--ce-text-light)" }}>Call the team now — lead with the trend:</strong> <em>"He was afebrile and 97% on room air yesterday. He's now 38.8, HR 112, BP 96/58, 92% on 2L, and his mental status has changed."</em></li>
              ...
              {scenarioIndex === 2 && (
                <>
                  <CopilotBlock label="Pattern Recognition">
                    Classic atypical ACS presentation in a woman. <strong style={{ color: "var(--ce-text-light)" }}>Pressure-quality chest discomfort with left shoulder radiation, diaphoresis, BP spike, HR trending up, SpO₂ drop</strong> — in a 64F with HTN, DM2, and a smoking history. Women frequently present with atypical symptoms, and "it resolved" does not rule out ACS. One negative troponin drawn 22 hours ago means nothing right now. The window for a first troponin to rise is 3–6 hours. You're outside that window and don't have a second value.
                  </CopilotBlock>
                  <CopilotBlock label="What Concerns Me Most">
                    Two things. First: the symptom description — pressure plus left shoulder radiation is a textbook anginal equivalent, regardless of how she's minimizing it. Second: <strong style={{ color: "var(--ce-text-light)" }}>she's downplaying it.</strong> Patients who minimize chest pain are often the ones who waited too long once before. The 6-month history of similar symptoms that was never worked up should be treated as a prior event until proven otherwise.
                  </CopilotBlock>
                  <CopilotBlock label="Assess Next — In This Order">
                    <ol style={{ margin: 0, paddingLeft: 18, display: "flex", flexDirection: "column", gap: 8 }}>
                      <li><strong style={{ color: "var(--ce-text-light)" }}>Pull the 12-lead and read it yourself</strong> — don't rely on the tech's read; look for ST changes, T-wave inversions, new Q waves</li>
            ...
            {/* The closing line */}
            <div style={{ borderTop: "1px solid var(--ce-line-dark)", paddingTop: 24, marginBottom: 40 }}>
              <p style={{
                fontFamily: "'IBM Plex Mono', monospace", fontSize: 13, fontWeight: 500,
                color: "var(--ce-teal)", lineHeight: 1.65, margin: 0,
              }}>
                {sc.closingLine}
              </p>
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
              <button className="sc-btn-ghost" onClick={() => setStep(2)} style={btnGhost}>
                ← Back
              </button>

              {/* Chain: 01 → 02 → 03 → app */}
              {scenarioIndex === 0 && (
                <button className="sc-btn-primary" onClick={() => switchScenario(1)} style={btnPrimary}>
                  Try Scenario 02 →
                </button>
              )}
              {scenarioIndex === 1 && (
                <button className="sc-btn-primary" onClick={() => switchScenario(2)} style={btnPrimary}>
                  Try Scenario 03 →
                </button>
              )}
              {scenarioIndex === 2 && (
